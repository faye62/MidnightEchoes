...
/**
 * 写一个工具函数用于获取昨日前七天的日期数组
 * @returns {Array} - 返回格式为 ['YYYY-MM-DD', 'YYYY-MM-DD', 'YYYY-MM-DD', 'YYYY-MM-DD', 'YYYY-MM-DD', 'YYYY-MM-DD', 'YYYY-MM-DD']
 * @param {number} day - 最近几天的日期，默认为 7 天
 * **/
export const getRecentSevenDays = (day: number = 7) => {
  const today = new Date();
  const days = [];
  for (let i = 0; i < day; i++) {
